...
# 打包后的ipa文件路径
backupIPA = '/Users/syf/Desktop/AutoArchive_HTCF/'

#项目所在的地址
projectPath = '/Users/syf/Store/恒天财富项目/电子合同正式分支/HtjfApp/HtjfApp'

plistPath = '/Users/syf/Desktop/AutoArchive_HTCF'
...
def cleanPro():
    ...
    if desDv == 1:
        desDvStr = 'Release'
        backupIPA = backupIPA + 'release'
    else:
        desDvStr = 'Debug'
        backupIPA = backupIPA + 'debug'
    # xcodeproj工程
    cleanProRun = 'xcodebuild clean -project %s.xcodeproj -scheme %s -configuration %s' % (projectPath, schemeName, desDvStr)
    # workspace工程
...
def archive():
    ...
    archiveRun = 'xcodebuild archive -project %s.xcodeproj -scheme %s -archivePath %s/%s.xcarchive' % (projectPath, schemeName, backupIPA, schemeName)
    # workspace工程
...
def exportIPA():
    ...
    # iOS9
    exportRun = 'xcodebuild -exportArchive -archivePath %s/%s.xcarchive -exportPath %s/%s -exportOptionsPlist %s/ExportOptions.plist' % (
    backupIPA, schemeName, backupIPA, schemeName,plistPath)
    print('++++++%s' % exportRun)
